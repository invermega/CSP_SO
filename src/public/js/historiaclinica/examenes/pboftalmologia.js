...
function Grabar() {
    ...
    console.log(obtenerDataRefraccion());
...
function obtenerDataParametros() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (input.length >= 2 && input[0].value.trim() !== '') {
            var codpru_id = input[0].value;
            var resultado_test = input[1].value;
...
function obtenerDataRefraccion() {
    var data = [];
    $('#Refraccion tbody tr').each(function () {
        
        var ojoDerecho = {
            'CILINDRO': fila.find('td:eq(2) input[type=text]').val(),
            'EJE': fila.find('td:eq(3) input[type=text]').val(),
            'AV': fila.find('td:eq(4) input[type=text]').val()
        };

        // Obtener los valores del ojo izquierdo
        var ojoIzquierdo = {
            'CILINDRO': fila.next().find('td:eq(2) input[type=text]').val(), // Cambiado el índice de columna
            'EJE': fila.next().find('td:eq(3) input[type=text]').val(), // Cambiado el índice de columna
            'AV': fila.next().find('td:eq(4) input[type=text]').val() // Cambiado el índice de columna
        };

        var dip = fila.find('td:last textarea').val();

        if (tipo === 'LEJOS' || tipo === 'CERCA (ADD)') {
            if (!data[tipo]) {
                data[tipo] = []; // Inicializar un array si no existe para este tipo
            }
            data[tipo].push({ // Agregar los datos al array correspondiente al tipo
                'OJO DERECHO': ojoDerecho,
                'OJO IZQUIERDO': ojoIzquierdo,
                'DIP': dip
            });
        }
    });
        ...
        success: function (result) {
            ...
            if (result[0].mensaje != 'sin datos') {
                ...
                if (result[0].agudeza_visual) {
                    ...
                    var tablaAgudeza = document.getElementById('agudeza_visual');
                    agudeza_visualArray.forEach(dato => {
                        console.log(dato);
                    });
                }
